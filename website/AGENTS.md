...
`website/` is the canonical Next.js/Supabase application. Do not reactivate the legacy `apps/order/` backend at repository root, restore client-side business writes, rewrite applied migrations, expose private documents, or enable fake providers in production. Preserve the existing uncommitted user work in the original Google Drive checkout. The published checkout is `/Users/jkang/.cache/eplate-canonical-publish`. A limited public release is deployed on Vercel; see docs/HANDOFF.md. No transactional service has been production-verified.
