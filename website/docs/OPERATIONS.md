...
Only with explicit approval: publish the tested release, update callbacks and change DNS/routes. Follow `../../apps/order/RETIREMENT.md`: restore/test D1/R2 export, freeze legacy writes, reconcile final delta and import reviewed data. The baseline D1 schema is retained in `apps/order/migrations/0001_initial.sql`; a reliable import still needs the real export. Local retirement safeguards do not disable an already deployed Worker. Ensure exactly one live writer. Verify health, login, upload, checkout, role boundaries and appointment completion after cutover.
