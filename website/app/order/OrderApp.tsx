...
export function OrderApp({ mode = 'customer', initialId = '' }: {
 ...
 {mode === 'customer' && role === 'customer' && <div className="panel"><h2>{order && ['draft', 'docs_pending', 'rejected'].includes(order.status) ? 'Resume order' : 'New order'}</h2><p>{formatMYR(PRICE_MYR)} installed package · {site.workshop.name}</p><form key={order?.id || 'new'} onSubmit={e => { e.preventDefault(); const data = Object.fromEntries(new FormData(e.currentTarget)); void run(() => action('draft', { ...data, vehicle_type_confirmed: data.vehicle_type_confirmed === 'on' }, order && ['draft', 'docs_pending', 'rejected'].includes(order.status) ? order.id : null)); }}>{[['owner_name', 'Owner name', editableOrder?.owner_name], ['whatsapp_phone', 'WhatsApp number (+60)', editableOrder?.customers?.whatsapp_phone], ['vehicle_registration', 'Vehicle registration', editableOrder?.vehicle_registration], ['chassis_vin', 'Chassis / VIN', editableOrder?.chassis_vin]].map(([name, label, value]) => <label className="field" key={name}>{label}<input name={name} defaultValue={value || ''} required maxLength={160}/></label>)}<label><input type="checkbox" name="vehicle_type_confirmed" required/> I confirm this vehicle is eligible for JPJePlate.</label><p><button className="btn" disabled={busy}>Save draft</button> <button type="button" onClick={() => { setSelected(''); setDetail(null); }}>Start another order</button></p></form></div>}
